...
interface RegularTemplateLoadModalProps {
  open: boolean;
  onClose: () => void;
  periodLabel: string;
  ...
  actionError?: string;
  onApply: (template: RegularWeeklyPlanTemplate) => Promise<void>;
...
export default function RegularTemplateLoadModal({
  open,
  onClose,
  periodLabel,
  templates,
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="regular-template-load-title"
      >
        <div className="border-b border-gray-200 p-6 dark:border-neutral-700">
          <h2
            id="regular-template-load-title"
            className="text-lg font-semibold text-gray-900 dark:text-gray-100"
          >
            템플릿 불러오기
          </h2>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            저장된 템플릿을 현재 평소 기간({periodLabel})에 적용합니다.
          </p>
        </div>
          ...
          {selectedTemplate ? (
            <div className="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-neutral-700 dark:bg-zinc-800/60">
              <p className="text-sm text-gray-700 dark:text-gray-200">
                「{selectedTemplate.name}」을 현재 평소 기간({periodLabel})에 적용합니다.
              </p>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
