...
export async function GET(request: NextRequest) {
  ...
  const res = await strapiFetch(
    `/api/user-profiles/vacation-weekly-plan-templates?${params}`,
    { jwt }
  );
...
export async function POST(request: NextRequest) {
  ...
  const res = await strapiFetch('/api/user-profiles/vacation-weekly-plan-templates', {
    method: 'POST',
    jwt,
