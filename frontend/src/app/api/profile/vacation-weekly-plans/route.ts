...
export async function GET(request: NextRequest) {
  ...
  const res = await strapiFetch(`/api/user-profiles/vacation-weekly-plans?${params}`, {
    jwt,
  });
  ...
  if (!res.ok) {
    ...
    const errorMessage =
      res.status === 403 && rawError === 'Forbidden'
        ...
        : (rawError ?? '방학기간 주차별 공부계획을 불러오지 못했습니다.');
...
export async function PUT(request: NextRequest) {
  ...
  const res = await strapiFetch('/api/user-profiles/vacation-weekly-plans', {
    method: 'PUT',
    jwt,
  ...
  if (!res.ok) {
    ...
    const errorMessage =
      res.status === 403 && rawError === 'Forbidden'
        ? '방학기간 주차별 공부계획 API 권한이 없습니다. 백엔드 서버를 재시작한 뒤 다시 시도해 주세요.'
        : (rawError ?? '방학기간 주차별 공부계획 저장에 실패했습니다.');
