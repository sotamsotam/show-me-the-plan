...
export default function WeeklyPlanItemList({
  ...
  function commitTitleEdit(itemId: string) {
    if (editingItemId !== itemId) {
      return;
    }

    const result = updateUnscheduledWeeklyPlanItemTitle(items, itemId, editingTitle);
    ...
    if ('error' in result) {
      ...
      setTitleEditError(result.error);
      return;
    }
  ...
  function startTitleEdit(itemId: string, title: string) {
    if (disabled) {
      return;
    }

    if (editingItemId && editingItemId !== itemId) {
      commitTitleEdit(editingItemId);
    }
  ...
  function handleTitleEditKeyDown(
    event: KeyboardEvent<HTMLInputElement>,
    itemId: string
  ) {
    if (event.key === 'Enter') {
      event.preventDefault();
      commitTitleEdit(itemId);
      return;
    }
      ...
      {items.length > 0 ? (
        <ul className="space-y-1.5">
          {items.map((item) => {
            ...
            const canDrag = isEditable && !disabled && !isEditing;
            ...
            return (
              <li
                ...
                className={`flex flex-wrap items-start gap-2 rounded-md border px-2 py-1.5 text-sm ${getWeeklyPlanItemRowSurfaceClasses(rowKind)} ${
                  dragUnscheduledIndex === unscheduledIndex
                    ? 'ring-2 ring-blue-400 ring-offset-1 dark:ring-offset-zinc-900'
                    : ''
                } ${isEditing ? 'ring-2 ring-blue-400 ring-offset-1 dark:ring-offset-zinc-900' : ''}`}
              >
                {isEditable ? (
                  <span
                    aria-hidden
                ...
                {isEditable && isEditing ? (
                  <input
                    ...
                    onBlur={() => commitTitleEdit(item.id)}
                    onKeyDown={(event) => handleTitleEditKeyDown(event, item.id)}
                    disabled={disabled}
                    autoFocus
                    maxLength={MAX_WEEKLY_PLAN_ITEM_TITLE_LENGTH}
                    aria-label={`${item.title} 제목 수정`}
                    aria-invalid={titleEditError ? true : undefined}
                    className="min-w-0 flex-1 rounded border border-gray-300 bg-white px-1 py-0.5 text-sm outline-none focus:border-blue-500 dark:border-neutral-600 dark:bg-zinc-900"
                  />
                ) : isEditable ? (
                  <button
                    type="button"
                    onClick={() => startTitleEdit(item.id, item.title)}
                    disabled={disabled}
                    className="min-w-0 flex-1 break-words text-left text-gray-800 hover:underline disabled:opacity-40 dark:text-gray-100"
                    title="클릭해 제목 수정"
                  >
                    {item.title}
                  </button>
