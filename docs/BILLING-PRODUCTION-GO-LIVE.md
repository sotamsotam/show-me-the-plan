...
| `PORTONE_API_SECRET` | 테스트 API 시크릿 | **라이브** API 시크릿 |
| `NEXT_PUBLIC_PORTONE_STORE_ID` | 테스트 스토어 ID | **라이브** 스토어 ID |
| `NEXT_PUBLIC_PORTONE_CHANNEL_KEY` | 테스트 채널 키 | **라이브** 채널 키 |
| `PORTONE_WEBHOOK_SECRET` | 웹훅 시크릿 | 포트원 콘솔에서 발급 |
| `PORTONE_WEBHOOK_SKIP_VERIFY` | `true` (로컬) | **`false`** |
...
응답에 `webhookUrl`, `cronUrl`이 포함됩니다. 포트원 콘솔 Webhook URL 등록 시 사용하세요.

---

## 4. 포트원 Webhook

1. [포트원 관리자 콘솔](https://admin.portone.io/integration-v2/manage/webhook?version=V2) → **웹훅 시크릿** 발급
2. Webhook URL 등록:
   ```
   https://your-domain.example.com/api/billing/webhooks/portone
   ```
3. `PORTONE_WEBHOOK_SKIP_VERIFY=false` 확인 후 테스트 결제 1건 → 서버 로그 `[billing] webhook.payment_succeeded` 확인
...
- 포트원 **테스트** 채널·키로 되돌리거나 Webhook 비활성화
- `PORTONE_WEBHOOK_SKIP_VERIFY=true`는 **임시**만 사용 (운영 상시 금지)
- Strapi subscription 수동 연장(CS 매뉴얼 참고)

---

*포트원 가맹·하위 PG 채널 계약, 통신판매업 신고 등 **계약/법무** 항목은 별도 일정으로 진행합니다.*
